#!/usr/bin/env python3
"""
GitHub Webhook Listener for Homelab Dashboard - Container Version

This service listens for GitHub webhook events and pulls the latest code
to deploy it to the homelab environment. It's designed to run in a Docker
...
Environment variables:
    ...
    REPO_PATH: Path to the repository (default: /app/repo)
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def _deploy_update(self):
        ...
        try:
            ...
            # When running in a container, use docker API or docker-compose within the host
            subprocess.run(['docker-compose', '-f', f'{repo_dir}/docker-compose.yml', 'up', '-d', '--build'], check=True)
